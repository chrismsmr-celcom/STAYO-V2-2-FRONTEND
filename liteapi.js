...
// ✅ Modification: Affichage du prix par nuit sur les marqueurs
...
// ==========================
// liteapi.js - STAYO Leaflet Final - OPTIMISÉ
// ==========================
...
async function callEngine(query) {
    ...
    try {
        ...
        // ✅ 1. Mise à jour PRIORITAIRE des paramètres globaux depuis la réponse de l'API
        if (data.context) {
            ...
            if (data.context.adults) currentSearchParams.adults = parseInt(data.context.adults, 10);
            if (data.context.currency) currentSearchParams.currency = data.context.currency;
            ratesCache.clear();
        }

        // ✅ 2. Calcul 100% Dynamique basé sur les nouvelles valeurs appliquées
        var nights = Math.max(1, Math.round((new Date(currentSearchParams.checkout) - new Date(currentSearchParams.checkin)) / 86400000));
        ...
        if (data.hotels && data.hotels.length > 0) {
            ...
            var cardsHtml = hotelsToShow.slice(0, 5).map(function(h, i) {
                ...
                if (h.price) {
                    if (hasUserDates) {
                        priceDisplay = Math.round(h.price) + ' ' + currentSearchParams.currency;
                    } else {
            ...
            if (data.context && data.context.suggested_activities && data.context.suggested_activities.length > 0) {
                var activitiesHtml = '<p style="margin-top:8px;font-size:12px;">Activités suggérées :</p><div class="ai-suggestions">' +
                    data.context.suggested_activities.slice(0, 4).map(function(a) {
                        return '<span class="ai-suggestion-chip" onclick="sendQuickReply(\'Activités ' + a + ' à ' + (data.context.event || '') + '\')">' + a + '</span>';
                    }).join('') + '</div>';
                appendMessage('bot', activitiesHtml);
            }
        } else {
            appendMessage('bot', data.message || "Aucun hôtel trouvé. Essayez de modifier vos dates ou votre budget.");
        }
    } catch (e) {
        var el = document.getElementById(loadingId);
        if (el) el.remove();
        appendMessage('bot', "Le serveur se réveille (hébergement gratuit). Réessayez dans quelques secondes.");
    } finally {
